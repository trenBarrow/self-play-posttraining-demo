#!/usr/bin/env python3
"""Portable helpers for strict run-scoped packet capture."""
...
def env_capture_interface() -> str:
    return os.environ.get("FPRIME_CAPTURE_INTERFACE", "").strip()


def env_capture_backend() -> str:
    return os.environ.get("FPRIME_CAPTURE_BACKEND", CAPTURE_BACKEND_AUTO).strip() or CAPTURE_BACKEND_AUTO
...
def run_docker(args: list[str], *, check: bool = False, text: bool = True) -> subprocess.CompletedProcess[str]:
    docker = docker_binary()
    if docker is None:
        raise SystemExit("Strict identity capture requires docker, but docker is not installed")
    proc = subprocess.run(
        [docker, *args],
        capture_output=True,
        text=text,
    ...
    if check and proc.returncode != 0:
        stderr = (proc.stderr or "").strip()
        raise SystemExit(f"docker command failed: {' '.join(args)}{': ' + stderr if stderr else ''}")
    return proc
...
def local_capture_command(path: Path, interface: str, filter_expr: str) -> list[str]:
    ...
    if shutil.which("sudo") is None:
        raise SystemExit(
            "tcpdump capture on the local Docker bridge requires root or passwordless sudo"
        )
    return ["sudo", "-n", *cmd]
...
def resolve_capture_backend(backend: str | None = None) -> str:
    ...
    if chosen not in SUPPORTED_CAPTURE_BACKENDS:
        raise SystemExit(
            f"Unsupported FPRIME_CAPTURE_BACKEND={chosen!r}. "
            f"Expected one of {sorted(SUPPORTED_CAPTURE_BACKENDS)}."
        )
    if chosen == CAPTURE_BACKEND_RDCTL_VM_BRIDGE:
        if not has_rdctl_vm():
            raise SystemExit("FPRIME_CAPTURE_BACKEND=rdctl_vm_bridge requires Rancher Desktop rdctl")
        return chosen
    if chosen == CAPTURE_BACKEND_LOCAL_DOCKER_BRIDGE:
        available, reason = local_docker_bridge_available()
        if not available:
            raise SystemExit(
                "FPRIME_CAPTURE_BACKEND=local_docker_bridge is unavailable: "
                f"{reason}"
            )
        return chosen
    if has_rdctl_vm():
        return CAPTURE_BACKEND_RDCTL_VM_BRIDGE
    available, _ = local_docker_bridge_available()
    if available:
    ...
    if sys.platform == "darwin":
        raise SystemExit(
            ...
            "Set FPRIME_CAPTURE_BACKEND and FPRIME_CAPTURE_INTERFACE explicitly if you have a supported backend."
        )
    raise SystemExit(
...
def preferred_capture_interface(backend: str) -> str | None:
    if backend == CAPTURE_BACKEND_RDCTL_VM_BRIDGE:
        return default_vm_bridge_interface()
    if backend == CAPTURE_BACKEND_LOCAL_DOCKER_BRIDGE:
        return default_local_bridge_interface()
    return None
...
def list_capture_interfaces(backend: str | None = None) -> list[str]:
    ...
    if proc.returncode != 0:
        raise SystemExit("tcpdump is required for generate but could not enumerate interfaces")
    interfaces = parse_capture_interfaces(proc.stdout)
    if not interfaces:
    ...
    *,
    ...
    if not chosen_interface:
        raise SystemExit(
            ...
            "Set FPRIME_CAPTURE_INTERFACE or resolve one before starting capture."
        )
